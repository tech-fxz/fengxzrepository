...
//TODO(misko): this function needs to be removed
     ...
     *
     * - `full` 每 `{string}` 每 Full version string, such as "0.9.18".
     * - `major` 每 `{number}` 每 Major version number, such as "0".
     * - `minor` 每 `{number}` 每 Minor version number, such as "9".
     * - `dot` 每 `{number}` 每 Dot version number, such as "18".
     * - `codeName` 每 `{string}` 每 Code name of the release, such as "jiggling-armfat".
     */
    var version = {
 ...
 *   });
     ...
     *     });
                 ...
                 * the `then` method to register callbacks, and these callbacks will receive a single argument 每
     ...
     * JSON vulnerability} allows third party website to turn your JSON resource URL into
                 ...
                 *
                 *    - **method** 每 `{string}` 每 HTTP method (e.g. 'GET', 'POST', etc)
                 *    - **url** 每 `{string}` 每 Absolute or relative URL of the resource that is being requested.
                 *    - **params** 每 `{Object.<string|Object>}` 每 Map of strings or objects which will be turned
                 *      to `?key1=value1&key2=value2` after the url. If the value is not a string, it will be
                 *      JSONified.
                 *    - **data** 每 `{string|Object}` 每 Data to be sent as the request message data.
                 *    - **headers** 每 `{Object}` 每 Map of strings or functions which return strings representing
                 *      HTTP headers to send to the server. If the return value of a function is null, the
                 *      header will not be sent.
                 *    - **xsrfHeaderName** 每 `{string}` 每 Name of HTTP header to populate with the XSRF token.
                 *    - **xsrfCookieName** 每 `{string}` 每 Name of cookie containing the XSRF token.
                 *    - **transformRequest** 每
                 *      `{function(data, headersGetter)|Array.<function(data, headersGetter)>}` 每
                 ...
                 *    - **cache** 每 `{boolean|Cache}` 每 If true, a default $http cache will be used to cache the
                 *      GET request, otherwise if a cache instance built with
                 *      {@link ng.$cacheFactory $cacheFactory}, this cache will be used for
                 *      caching.
                 *    - **timeout** 每 `{number|Promise}` 每 timeout in milliseconds, or {@link ng.$q promise}
     ...
     *      requests with credentials} for more information.
                 ...
                 *
                 *   - **data** 每 `{string|Object}` 每 The response body transformed with the transform
                 *     functions.
                 *   - **status** 每 `{number}` 每 HTTP status code of the response.
                 *   - **headers** 每 `{function([headerName])}` 每 Header getter function.
                 *   - **config** 每 `{Object}` 每 The configuration object that was used to generate the request.
                 *
 ...
 * $httpBackend} which can be trained with responses.
   ...
   *
     ...
     * only public api is:
     *
     * * `id` 每 `{string}` 每 locale id formatted as `languageId-countryId` (e.g. `en-us`)
     */
...
///////////////////////////////////
     ...
     *
     *    * `context` 每 `{object}` 每 an object against which any expressions embedded in the strings
     *      are evaluated against (typically a scope object).
     *    * `locals` 每 `{object=}` 每 local variables context object, useful for overriding values in
     *      `context`.
     *
     *    The returned function also has the following properties:
     *      * `literal` 每 `{boolean}` 每 whether the expression's top-level node is a JavaScript
     *        literal.
     *      * `constant` 每 `{boolean}` 每 whether the expression is made entirely of JavaScript
     *        constant literals.
     *      * `assign` 每 `{?function(context, value)}` 每 if the expression is assignable, this will be
 ...
 *   });
     ...
     * **Methods**
     *
     * - `resolve(value)` 每 resolves the derived promise with the `value`. If the value is a rejection
     *   constructed via `$q.reject`, the promise will be rejected instead.
     * - `reject(reason)` 每 rejects the derived promise with the `reason`. This is equivalent to
     ...
     * **Properties**
     *
     * - promise 每 `{Promise}` 每 promise object associated with this deferred.
     *
     *
     * # The Promise API
     *
     ...
     * **Methods**
     *
     * - `then(successCallback, errorCallback, notifyCallback)` 每 regardless of when the promise was or
     ...
     *   method.
     *
     * - `catch(errorCallback)` 每 shorthand for `promise.then(null, errorCallback)`
     *
     * - `finally(callback)` 每 allows you to observe either the fulfillment or rejection of a promise,
 ...
 *                      'other': '{{person1}}, {{person2}} and {} other people are viewing.'}">
     ...
     *
     *   * `variable in expression` 每 where variable is the user defined loop variable and `expression`
     *     is a scope expression giving the collection to enumerate.
     *
     *     For example: `album in artist.albums`.
     *
     *   * `(key, value) in expression` 每 where `key` and `value` can be any user defined identifiers,
     *     and `expression` is the scope expression giving the collection to enumerate.
     *
     *     For example: `(name, age) in {'adam':10, 'amalie':12}`.
     *
     *   * `variable in expression track by tracking_expression` 每 You can also provide an optional tracking function
